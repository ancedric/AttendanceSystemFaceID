...
Public Class SignupScreen
    ...
    Private Sub SignupButton_Click(sender As Object, e As RoutedEventArgs) Handles signupButton.Click
        ...
        Dim admin As New Admin(nom, prenom, email, pwd)

        If (email IsNot Nothing AndAlso password IsNot Nothing) Then
            Dim loggedIn As Boolean = admin.SignUp(nom, prenom, email, pwd)
            If (loggedIn = True) Then
                Dim login As New loginScreen
                Me.Close()
                login.Show()
            Else
                MsgBox("L'inscription a échoué!")
            End If
        Else
            MsgBox("Veuillez remplir tous les champs s'il vous plait !")
        End If
    End Sub
    Private Sub Mouse_Click(sender As Object, e As MouseButtonEventArgs) Handles logInBtn.MouseLeftButtonDown
        Dim login As New loginScreen
        Me.Close()
        login.Show()
    End Sub
